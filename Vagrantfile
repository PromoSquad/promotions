...
Vagrant.configure(2) do |config|
  ...
  config.vm.network "forwarded_port", guest: 5432, host: 5001, host_ip: "127.0.0.1"
  ...
  config.vm.provision "shell", inline: <<-SHELL
    ...
    apt-get update
    apt-get install -y git vim tree wget vim jq python3 python3-dev python3-pip python3-venv python3-selenium
  ...
  config.vm.provision "shell", inline: <<-SHELL
    ...
    echo "ibmcloud target --cf -o yk2494@nyu.edu -s dev"
    echo "\n************************************"
  SHELL
